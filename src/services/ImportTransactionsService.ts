...
class ImportTransactionsService {
  async execute(filePath: string): Promise<Transaction[]> {
    ...
    const existentCategories = categoryExists.map(
      (category: Category) => category.title,
    );

    const classCategory = plainToClass(Category, existentCategories);
    ...
    const finalCategories = [...newCategories, ...classCategory];
